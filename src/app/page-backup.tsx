...
export default function Home() {
  ...
  const testGetTasks = async () => {
    ...
    try {
      ...
      localStorage.setItem('token', 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJ1c2VySWQiOiI2ODg1MTA2MGJhNzBkY2M2MjZkYzdjYmUiLCJlbWFpbCI6InBydWViYUBlamVtcGxvLmNvbSIsImlhdCI6MTc1MzU1MjY3MCwiZXhwIjoxNzU0MTU3NDcwfQ.s73LvGgpPDZP7bJ0piWiK1iarerqclJKzLihRvU5keM');
                ...
                <div key={task._id} className="mb-2 p-3 bg-white rounded border">
                  ...
                  <div className="flex gap-2 mt-2">
                    <span className={`px-2 py-1 rounded text-xs ${
                      ...
                      task.status === 'in-progress' ? 'bg-blue-200' : 'bg-green-200'
                    }`}>
                      {task.status}
                    </span>
                    <span className={`px-2 py-1 rounded text-xs ${
                      task.priority === 'high' ? 'bg-red-200' :
                      task.priority === 'medium' ? 'bg-yellow-200' : 'bg-gray-200'
                    }`}>
                      {task.priority}
                    </span>
                  </div>
  ...
  const testGetTasks = async () => {
    ...
    try {
      ...
      localStorage.setItem(
        "token",
        "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJ1c2VySWQiOiI2ODg1MTA2MGJhNzBkY2M2MjZkYzdjYmUiLCJlbWFpbCI6InBydWViYUBlamVtcGxvLmNvbSIsImlhdCI6MTc1MzU1MDk1NywiZXhwIjoxNzU0MTU1NzU3fQ.XmcyNghGsJ235JjN2rJrqZcoFiF_2ei-isrFfrNi98M",
      );
